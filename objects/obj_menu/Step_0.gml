...
if (back_key)
{
    if (global.pause)
    {
        if (page != 0)
        {
            page = 0;
            menu_option[page] = 0;
            audio_play_sound(snd_menu_opt_selected_chime, global.sfx_vol, false, 0.5, 0, 0.4);
        }
        else
        {
    ...
    {
        audio_pause_all();
        audio_play_sound(snd_menu_opt_selected_chime, global.sfx_vol, false, 0.5, 0, 1);
        audio_play_sound(snd_intro_credit_menu, global.music_vol, true, 1);
...
if (inputting)
{
	    ...
	    switch (ds_grid[# 1, menu_option[page]])
    {
		...
		if(hinput != 0){
				audio_play_sound(snd_menu_opt_switch_chime, 1, false, 0.25, 0, 1.25);
